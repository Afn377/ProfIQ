...
def _prefix_range_upper_bound(prefix: str) -> str:
    ...
    exactly the strings starting with ``prefix`` — this lets the query use an
    index on the expression as a range scan; a plain ``LIKE 'prefix%'`` on the
    same expression does not (verified: CockroachDB's optimizer does not
    rewrite it into an index span the way it does for an explicit range).
    """
    if not prefix:
        return prefix
...
def platform_summary(request):
    ...
    Optional ``?institution=<name>`` scopes every stat (professor count, review
    count, top professors and department counts) to that institution, matched
