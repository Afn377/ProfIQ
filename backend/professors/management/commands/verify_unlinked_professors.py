"""Backfill ``external_ref`` for ``Professor`` rows never linked to RMP.

Walks unlinked rows grouped by ``institution`` (one school lookup per
institution instead of per professor), searches RMP by name, and on a confident
fuzzy match updates the row with the RMP legacy id and stats. Rows are only
ever updated in place — nothing is created or deleted.
"""
...
def _name_matches(name: str, teacher: RMPTeacher) -> bool:
    """Same fuzzy name-match rule as ``scrapers.rmp.scrape_rmp``: a candidate
    matches when one casefolded name contains the other, and empty names never
    match."""
